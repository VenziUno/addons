...
class TiktokShop(models.Model):
    ...
    def action_get_cipher(self):
        ...
        for shop in self:
            ...
            base_url = f"{domain}/authorization/202309/shops?app_key={app}&access_token={access_token}&timestamp={timest}"
            url = f"{base_url}"
            ...
            if values.get('data'):
                ...
                shop.write({
                    'tiktok_chiper': data.get('shops')[0].get('cipher')
                })

        def auto_action_get_access_token_tiktok(self):
            shop_ids = self.search([()])
            for shop in shop_ids:
                shop.action_get_access_token_tiktok()