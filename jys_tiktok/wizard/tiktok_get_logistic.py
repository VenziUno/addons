...
class TiktokGetLogistic(models.TransientModel):
    ...
    is_warehouse = fields.Boolean('Create Warehouse', default=True)

    def action_confirm(self):
        logistic_obj = self.env['delivery.carrier']
        tt_warehouse_obj = self.env['tiktok.warehouse']
        ...
        for wh in values.get('data').get('warehouses'): 
            ...
            if self.is_warehouse:
                warehouse = tt_warehouse_obj.search([('warehouse_id','=', warehouse_id)], limit=1)
                if warehouse:
                    continue
                address = wh.get('address')
                tt_warehouse_obj.create({
                    'name': wh.get('name'),
                    'warehouse_id': wh.get('id'),
                    'address': f"{address.get('full_address')}, {address.get('city')}, {address.get('region')}"
                })
